...
//  * pattern matching - the if statement can use the "is" keyword in combination with
//  * declaring a local variable to make your code safer.

// add and remove the "" to change between string and int
object o = 3; // "3" will result to execution of else statement as it's string
int j = 4;
if(o is int i){
    WriteLine($"{i} x {j} = {i * j}");
}else {
...
var animals = new Animal?[]{
    new Cat { Name = "Karen", Born =  new(year: 2022, month: 8, day: 23), Legs = 4, IsDomestic = true }, null,
...
foreach(Animal? animal in animals) {
    ...
    // ! switch expression syntax using lambdas
